...
const _authInvalid = '{"ok":false,"error_code":"auth_invalid",'
    '"message":"Invalid user app token format."}';
const _authUnknown = '{"ok":false,"error_code":"auth_unknown",'
...
void main() {
  ...
  group('the token walkthrough', () {
    testWidgets('explains what the token is and where it lives',
      ...
      expect(find.textContaining('not an account you log into here'),
          findsOneWidget);
    ...
    testWidgets('carries the attribution their terms require', (tester) async {
      await _pump(tester);
      expect(find.textContaining(RepeaterBookClient.attributionLine),
          findsOneWidget);
    });
  });
