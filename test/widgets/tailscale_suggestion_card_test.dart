...
void main() {
  ...
  testWidgets('suggests Tailscale for .local addresses', (tester) async {
    await tester.pumpWidget(
      _wrap(const TailscaleSuggestionCard(kind: HaUrlKind.mdnsLocal)),
    );
    expect(
      find.textContaining('only works on your home network'),
      findsOneWidget,
  ...
  testWidgets('confirms a detected tailnet address', (tester) async {
    await tester.pumpWidget(
      _wrap(const TailscaleSuggestionCard(kind: HaUrlKind.tailscale)),
    );
  ...
  testWidgets('warns about public plain HTTP', (tester) async {
    await tester.pumpWidget(
      _wrap(
        TailscaleSuggestionCard(kind: HaUrlKind.publicHttp, onLearnMore: () {}),
      ),
    );
  ...
  testWidgets('uses theme-derived colors so text is legible in dark mode', (
    ...
    final body = tester.widget<Text>(
      find.textContaining('gives Home Assistant'),
    );
  ...
  testWidgets('renders nothing for HTTPS and invalid input', (tester) async {
    await tester.pumpWidget(
      _wrap(const TailscaleSuggestionCard(kind: HaUrlKind.publicHttps)),
    );
    expect(find.byType(Card), findsNothing);

    await tester.pumpWidget(
      _wrap(const TailscaleSuggestionCard(kind: HaUrlKind.invalid)),
    );
    expect(find.byType(Card), findsNothing);
  });
}
