...
void main() {
  ...
  Future<void> pump(WidgetTester tester, SafetyAdvisoryDto advisory,
      {String ackKey = 'dev-1'}) async {
  ...
  testWidgets('no acknowledgement required: banner and controls show together',
      (tester) async {
  ...
  testWidgets('the banner offers both the live and archived safety links',
    ...
    await pump(tester, _advisory(acknowledgeRequired: true));
    expect(find.widgetWithText(OutlinedButton, 'Safety instructions'),
        findsOneWidget);
    expect(
        find.widgetWithText(OutlinedButton, 'Archived copy'), findsOneWidget);
  });

  testWidgets('severity drives the banner label', (tester) async {
    await pump(
        tester, _advisory(acknowledgeRequired: false, severity: 'warning'));
