...
void main() {
  ...
  group('quantizeFrame', () {
    ...
    test('a frame already within the palette keeps its identity', () {
      ...
      expect(
        identical(quantizeFrame(frame), frame),
        isTrue,
  ...
  group('save to device (stored microapp)', () {
    ...
    testWidgets('the Text kind reveals a text field and blocks an empty save', (
      ...
      await tester.pumpAndSettle();
      expect(
        find.byKey(const Key('stored-text-field')),
        findsOneWidget,
        reason: 'dialog stays open on empty text',
      );
    });

    testWidgets('saving an animation stores each frame as a still and loops them', (
      tester,
    ) async {
      ...
      expect(
        codec.encodeStoredCalls,
        hasLength(2),
        reason: 'each frame is stored as its own still',
      );
      expect(
        codec.encodeStoredCalls.every((c) => c.scroll == 'none'),
        isTrue,
        reason: 'a frame is a plain still, not a scroll',
      );
      expect(
        codec.encodeAnimationCalls,
        isEmpty,
    ...
    testWidgets('a silent device is reported as unconfirmed, not "now playing"', (
      ...
      expect(
        find.textContaining('now playing'),
        findsNothing,
    ...
    testWidgets('two replays of the same design use distinct sequences', (
      ...
      expect(
        codec.encodeStoredPlayCalls[0].cid,
        codec.encodeStoredPlayCalls[1].cid,
        reason: 'same design',
      );
      expect(
        codec.encodeStoredPlayCalls[0].sequence,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          codec.encodeStoredPlayCalls.map((c) => c.cid),
          [cid],
        ...
        expect(ble.writes.map((w) => w.charUuid).skip(writesBefore), [
          'ddp',
          'ddp',
        ]);
        expect(
    ...
    testWidgets(
      'pinning an animation default re-sets the loop and repeats it',
      (tester) async {
        final codec = FakeSpecCodec();
        final ble = FakeBleService();
        await tester.pumpWidget(
    ...
    testWidgets('re-saving identical content re-uses the same device slot', (
      ...
      expect(
        ...
        cids[0],
        reason:
            'identical content must overwrite the same stored id, '
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final codec = FakeSpecCodec()
        ..advertisedResolutionResult = const PanelResolutionDto(
          width: 20,
          height: 20,
        );
      await tester.pumpWidget(
  ...
  testWidgets('with no advertisement or cache, DeviceInfo sizes the canvas', (
    ...
    final codec = FakeSpecCodec()
      ...
      ..deviceInfoResolutionResult = const PanelResolutionDto(
        width: 20,
        height: 20,
      );
