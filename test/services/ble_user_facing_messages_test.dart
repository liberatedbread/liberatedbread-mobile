...
void main() {
  group('BlePairingRequiredException wording follows the platform', () {
    test(
      'Apple platforms are pointed at the system alert, not at Settings',
      () {
        final message = BlePairingRequiredException.forPlatform(
          isApple: true,
        ).message;
        expect(
          message,
          ...
          reason:
              'On iOS and macOS the prompt is an alert CoreBluetooth puts '
        ...
        expect(
          ...
          reason:
              'An unbonded BLE peripheral does not appear under Settings > '
    ...
    test('Android and BlueZ keep the settings wording', () {
      ...
      expect(
        message,
        equals(const BlePairingRequiredException().message),
        reason:
            'The non-Apple form must stay the default, so nothing that '
    ...
    test('both forms still say what the user has to do', () {
      for (final isApple in [true, false]) {
        expect(
          ...
          contains('try again'),
          reason:
              'The retry is the actionable half; a refusal that only '
  ...
  group('a forgotten Apple identifier is not a proximity problem', () {
    test('the message says to scan, not to move closer', () {
      ...
      expect(
        ...
        contains('scan'),
        reason:
            'On Apple platforms a device id is a system-minted UUID, not '
      ...
      expect(
        ...
        reason:
            'The generic connect-failure wording is what this type exists '
            'to replace.',
      );
    });
    ...
    test('it says what state the device has to be in', () {
      ...
      expect(
        ...
        contains('range'),
        reason:
            'A sighting needs the device advertising and within radio '
    ...
    test('it is a UserFacingException, so the UI renders it verbatim', () {
      expect(
        ...
        reason:
            'Otherwise friendlyErrorText falls back to a generic string '
