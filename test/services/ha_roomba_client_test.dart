...
void main() {
  ...
  group('state translation', () {
    ...
    test('falls back to the canonical state when there is no status', () {
      final fields = HaRoombaClient.stateFields(
        _vacuum(state: 'returning', attributes: const {'battery_level': 55}),
      );
    ...
    test(
      'reads bin-full off the vacuum when the integration puts it there',
      () {
        final fields = HaRoombaClient.stateFields(
          _vacuum(attributes: const {'battery_level': 94, 'bin_full': false}),
        );
    ...
    test('finds the bin sensor belonging to this robot, not another', () {
      final sensors = [
        const HaEntityState(
          entityId: 'binary_sensor.downstairs_bin_full',
          state: 'on',
        ),
        const HaEntityState(
          entityId: 'binary_sensor.dorita_bin_full',
  ...
  group('commands', () {
    test('each spec command maps onto its vacuum service', () async {
      ...
      expect(api.serviceCalls.map((c) => c.service).toList(), [
        'start',
        'pause',
        'stop',
        'return_to_base',
      ...
      expect(
        api.serviceCalls.every((c) => c.entityId == 'vacuum.dorita'),
        isTrue,
      );
    });
  ...
  group('supported_features', () {
    test('hides what the entity does not advertise', () {
      ...
      final entity = _vacuum(
        attributes: const {'supported_features': 8192 | 16},
      );
  ...
  group('HaRoombaController', () {
    ...
    test(
      'an entity HA no longer has says so, rather than looking offline',
      () async {
        api.entities = {};
        final controller = HaRoombaController(
          client: client,
        ...
        final error = controller.state.first.then<Object?>(
          (_) => null,
          onError: (Object e) => e,
        );
        await controller.connect();
    ...
    test(
      'only fetches binary_sensors when the vacuum lacks the reading',
      () async {
        api.entities = {
          'vacuum.dorita': _vacuum(
            attributes: const {'battery_level': 94, 'bin_full': true},
          ),
        };
        final controller = HaRoombaController(
          client: client,
