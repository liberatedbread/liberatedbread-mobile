...
void main() {
  ...
  FakeRepeaterSource sourceWithOne() => FakeRepeaterSource(
    id: 'test',
    byState: {
      'CT': [
  ...
  test(
    'an equal-but-new request reuses the result rather than refetching',
    () async {
  ...
  test('a different request is a different entry', () async {
    ...
    await container.read(
      radioSuggestionProvider(request(radiusKm: 160)).future,
    );
  ...
  test(
    'a failing source surfaces as a result, not as a provider error',
    () async {
  ...
  test('the service provider is wired to the real sources by default', () {
    ...
    addTearDown(container.dispose);
    expect(
      container.read(channelSuggestionServiceProvider),
