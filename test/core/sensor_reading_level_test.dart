...
void main() {
  group('radon (matched by unit)', () {
    test('bands at the Airthings/WHO 100/150 thresholds', () {
      expect(
        sensorReadingLevel(unit: 'Bq/m³', value: 55),
        SensorReadingLevel.good,
      );
      expect(
        sensorReadingLevel(unit: 'Bq/m³', value: 99.9),
        SensorReadingLevel.good,
      );
      expect(
        sensorReadingLevel(unit: 'Bq/m³', value: 100),
        SensorReadingLevel.fair,
      );
      expect(
        sensorReadingLevel(unit: 'Bq/m³', value: 149),
        SensorReadingLevel.fair,
      );
      expect(
        sensorReadingLevel(unit: 'Bq/m³', value: 150),
    ...
    test('accepts the ASCII spelling of the unit', () {
      expect(
        sensorReadingLevel(unit: 'Bq/m3', value: 200),
        SensorReadingLevel.poor,
      );
    });

    test(
      'a radon reading mislabeled with a VOC class still gets radon bands',
      () {
  ...
  group('VOC', () {
    test('bands ppb at the Airthings 250/2000 defaults', () {
      for (final klass in [
        ...
        expect(
          sensorReadingLevel(deviceClass: klass, unit: 'ppb', value: 120),
          SensorReadingLevel.good,
        );
        expect(
          sensorReadingLevel(deviceClass: klass, unit: 'ppb', value: 250),
          SensorReadingLevel.fair,
        );
        expect(
          sensorReadingLevel(deviceClass: klass, unit: 'ppb', value: 2000),
          SensorReadingLevel.poor,
        );
      }
    });
  ...
  group('humidity', () {
    test('bands at the Airthings 25/30/60/70 defaults', () {
      expect(
        sensorReadingLevel(deviceClass: 'humidity', unit: '%', value: 20),
        SensorReadingLevel.poor,
      );
      expect(
        sensorReadingLevel(deviceClass: 'humidity', unit: '%', value: 27),
        SensorReadingLevel.fair,
      );
      expect(
        sensorReadingLevel(deviceClass: 'humidity', unit: '%', value: 45),
        SensorReadingLevel.good,
      );
      expect(
        sensorReadingLevel(deviceClass: 'humidity', unit: '%', value: 65),
        SensorReadingLevel.fair,
      );
      expect(
        sensorReadingLevel(deviceClass: 'humidity', unit: '%', value: 75),
        SensorReadingLevel.poor,
      );
    });
  ...
  group('battery', () {
    test('bands at the usual 20/10 warning rungs', () {
      expect(
        sensorReadingLevel(deviceClass: 'battery', unit: '%', value: 85),
        SensorReadingLevel.good,
      );
      expect(
        sensorReadingLevel(deviceClass: 'battery', unit: '%', value: 20),
        SensorReadingLevel.fair,
      );
      expect(
        sensorReadingLevel(deviceClass: 'battery', unit: '%', value: 10),
        SensorReadingLevel.poor,
      );
    });
  ...
  group('honest silence', () {
    test('unbanded quantities get no verdict', () {
      ...
      expect(
        sensorReadingLevel(deviceClass: null, unit: null, value: 5),
        isNull,
      );
    });
    ...
    test('a missing or non-finite value gets no verdict', () {
      expect(
        sensorReadingLevel(deviceClass: 'battery', unit: '%', value: null),
        isNull,
      );
      expect(
        sensorReadingLevel(
