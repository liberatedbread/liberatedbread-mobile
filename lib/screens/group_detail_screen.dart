...
import 'groups_screen.dart' show GroupTile, pluralCategoryLabel;
...
class _GroupDetailScreenState extends ConsumerState<GroupDetailScreen> {
  ...
  Future<void> _run(
    ...
    setState(() {
      ...
      for (final member in members) {
        _latest[member.id] = GroupRunEvent(
          deviceId: member.id,
          status: GroupDeviceStatus.queued,
        );
      }
    });
    _stop = StopSignal();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: members.isEmpty
            ? _EmptyMembers(waiting: membersAsync.isLoading)
            : ListView(
                ...
                children: [
                  ...
                  for (final member in members) ...[
                    _MemberRow(member: member, event: _latest[member.id]),
  ...
  ({String title, List<String> deviceIds})? _resolveGroup() {
    final category = widget.category;
    if (category != null) {
      final auto = ref.watch(autoGroupsProvider).valueOrNull;
      final group =
          auto?.groups.where((g) => g.category == category).firstOrNull;
      return (
        title: pluralCategoryLabel(category),
        deviceIds: [for (final d in group?.devices ?? const []) d.id],
      );
    }
    final group = ref
    ...
    return (title: group.name, deviceIds: group.deviceIds);
  }
}
...
class _OpButtons extends StatelessWidget {
  ...
  int _supportCount(GroupOp op) => members
      .where((m) => m.spec != null && supportedGroupOps(m.spec!).contains(op))
      .length;
  ...
  Widget build(BuildContext context) {
    ...
    Widget opButton(GroupOp op, {VoidCallback? custom}) {
      // Command ops need at least one member whose spec resolves the verb.
      // Battery is always worth attempting: the SIG service needs no spec,
      // so a spec-less member can still answer. Sensors need a spec.
      final supported = _supportCount(op);
      final enabled = switch (op) {
        GroupOp.readBattery => true,
        _ => supported > 0,
      };
      final count = op == GroupOp.readBattery
          ? null
...
class _MemberRow extends StatelessWidget {
  ...
  const _MemberRow({required this.member, this.event});

  String _preRunSummary() {
    final spec = member.spec;
    if (spec == null) return 'Kind unknown — will match on connect';
    final ops = supportedGroupOps(spec);
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = switch (current?.status) {
    ...
    return GroupTile(
      icon: category?.icon ?? Icons.bluetooth,
      title: member.name.isNotEmpty ? member.name : 'Unknown device',
      subtitle: subtitle,
      onTap: null,
      trailing: _StatusGlyph(status: current?.status),
    );
  }
}
