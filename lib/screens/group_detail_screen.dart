...
class _GroupDetailScreenState extends ConsumerState<GroupDetailScreen> {
  ...
  /// without an entry show their pre-run summary instead. Keyed by member
  /// id, which is bare for BLE members and `net:`-prefixed for Wi-Fi ones,
  /// so the two runners' events can share one map without colliding.
  final Map<String, GroupRunEvent> _latest = {};
  final List<StreamSubscription<GroupRunEvent>> _runSubs = [];

  /// Streams still emitting for the current run — the buttons re-enable
  /// only when the LAST one finishes, whichever transport that is.
  int _liveStreams = 0;
  ...
  void dispose() {
    ...
    // started, and cancelling the subscriptions makes the BLE runner's
    // finally disconnect whichever device was mid-flight (network sends
    // in flight just finish — they are single short exchanges).
    _stop.stop();
  ...
  Future<void> _pickBrightness(GroupMembers members) async {
    var percent = 100.0;
  ...
  ({
    ...
    if (category != null) {
      ...
      return (
        ...
        deviceIds: bucket?.memberIds ?? const [],
        group: null,
...
class _OpButtons extends StatelessWidget {
  ...
  int _supportCount(GroupOp op) =>
      members.ble.where((m) => m.specOps.contains(op)).length +
      members.network.where((m) => m.ops.contains(op)).length;

  @override
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final text = Theme.of(context).textTheme;
    // Only a BLE member's spec can be unknown-until-connect; a network
    // member's controls either resolved or they did not.
    final anySpecUnknown = members.ble.any((m) => m.spec == null);

    Widget opButton(GroupOp op, {VoidCallback? custom}) {
      // An op that works spec-less (battery, via the SIG service) is always
      // worth attempting — though only BLE members can, so it needs one. The
      // rest need at least one member whose spec resolves the verb — or a
      // member with no spec yet, whose row already promises "will match on
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Bluetooth devices are handled one at a time — each connects, '
          'acts, and disconnects before the next. Wi-Fi devices run '
          'alongside, a few at once.',
...
/// One Wi-Fi member row — [_MemberRow]'s network sibling. Separate because
/// the pre-run story differs: there is no "will match on connect" (network
/// controls resolve before the run or not at all), and the icon comes from
/// the recorded category rather than a spec object.
class _NetworkMemberRow extends StatelessWidget {
  final NetworkGroupMember member;
  final GroupRunEvent? event;
  final bool queued;
  ...
  Widget build(BuildContext context) {
    ...
    final status =
        current?.status ?? (queued ? GroupDeviceStatus.queued : null);
    final category = DeviceCategory.parse(member.category);
