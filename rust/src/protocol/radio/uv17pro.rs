...
use super::{read_reply_len, Block, ACK, REPLY_HEADER_LEN};
...
pub const HANDSHAKE_SEND: HandshakeExchange = HandshakeExchange {
    request: &[
...
///
/// The published table has twenty four-byte symbols, and every radio in this
/// family uses the second, so that is the one kept. It is a rotating XOR
/// with four exemptions, and those exemptions are what make it its own
/// inverse -- see [`crypt`].
const SYMBOL: &[u8; 4] = b"CO 7";
...
pub fn parse_read_reply(reply: &[u8], addr: u16, len: u8) -> Result<Vec<u8>, ProtocolError> {
    let expected = read_reply_len(len);
    ...
    Ok(crypt(&reply[REPLY_HEADER_LEN..]))
}
...
pub fn read_plan(regions: &[(u16, u16)], block_size: u16) -> Vec<Block> {
...
mod tests {
    ...
    fn a_read_reply_round_trips_through_the_parser() {
        ...
        let scrambled = crypt(&payload);
        let mut reply = vec![CMD_READ, 0x12, 0x34, 0x40];
        reply.extend_from_slice(&scrambled);

        let parsed = parse_read_reply(&reply, 0x1234, 0x40).unwrap();
    ...
    fn a_reply_for_another_address_is_refused() {
        ...
        assert!(parse_read_reply(&reply, 0x9000, 0x40).is_err());
    }
    ...
    fn a_short_or_long_reply_is_refused() {
        ...
        assert!(parse_read_reply(&short, 0x1234, 0x40).is_err());

        let mut long = vec![CMD_READ, 0x12, 0x34, 0x40];
        long.extend_from_slice(&[0u8; 0x80]);
        assert!(parse_read_reply(&long, 0x1234, 0x40).is_err());
    }
    ...
    fn a_reply_with_the_wrong_opcode_is_refused() {
        ...
        reply.extend_from_slice(&[0u8; 0x40]);
        assert!(parse_read_reply(&reply, 0x1234, 0x40).is_err());
    }

    #[test]
    fn the_expected_reply_length_includes_the_header() {
        assert_eq!(read_reply_len(0x40), 0x44);
        assert_eq!(read_reply_len(0x80), 0x84);
    }
    ...
    fn a_read_plan_covers_exactly_the_image() {
        ...
        let total: usize = plan.iter().map(|b| usize::from(b.len)).sum();
